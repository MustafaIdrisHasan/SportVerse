...
import QuickTools from '../navigation/QuickTools';

interface ResizableQuickToolsProps {
  defaultWidth?: number;
  minWidth?: number;
  maxWidth?: number;
  className?: string;
...
const ResizableQuickTools: React.FC<ResizableQuickToolsProps> = ({
  defaultWidth = 320,
  minWidth = 240,
  maxWidth = 600,
  className = '',
  ...
  return (
    <>
      ...
      <div
        ref={sidebarRef}
        className={`fixed right-0 bg-dark-100/50 backdrop-blur-lg border-l border-gray-600/30 transition-all duration-300 ease-in-out shadow-xl ${className}`}
        style={{
          width: currentWidth,
        ...
        {isCollapsed && (
          ...
          >
            ...
            <div className="text-center mb-4">
              <button
                onClick={toggleCollapse}
                className="p-3 rounded-xl bg-gradient-to-br from-accent-electric/20 to-accent-electric/10 border border-accent-electric/30 hover:border-accent-electric/50 transition-all duration-200 group" 
                title="Expand Quick Tools"
              >
                <svg className="w-6 h-6 text-accent-electric group-hover:text-white transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z" />
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                </svg>
              </button>
...
export default ResizableQuickTools; 